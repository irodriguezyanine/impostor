/**
 * Categorías del juego Impostor Chile
 *
 * Edita este archivo para agregar más categorías y palabras.
 * Puedes agregar cientos de palabras por categoría.
 *
 * Al agregar una nueva categoría:
 * 1. Añade el objeto con id, name, icon y words
 * 2. Agrega la traducción en src/lib/i18n.ts en TRANSLATIONS[*].categories[id]
 */
export type Category = {
  id: string;
  name: string;
  icon: string; // Emoji o nombre de icono
  iconImage?: string; // Ruta opcional a imagen (ej: /logo-alamicos.png)
  words: string[]; // Las palabras secretas
  wordHints?: Record<string, string>; // Pista relacionada por palabra (para impostores)
};

export const CATEGORIES: Category[] = [
  {
    id: "futbolistas-internacionales",
    name: "Futbolistas Internacionales (Masculino)",
    icon: "⚽",
    words: [
      "Lionel Messi",
      "Cristiano Ronaldo",
      "Pelé",
      "Diego Maradona",
      "Kylian Mbappé",
      "Erling Haaland",
      "Neymar Jr.",
      "Zinedine Zidane",
      "Ronaldo Nazário (El Fenómeno)",
      "Ronaldinho",
      "Johan Cruyff",
      "Franz Beckenbauer",
      "Zlatan Ibrahimović",
      "Robert Lewandowski",
      "Kevin De Bruyne",
      "Luka Modrić",
      "Vinícius Jr.",
      "Jude Bellingham",
      "Harry Kane",
      "Mohamed Salah",
      "Andrés Iniesta",
      "Xavi Hernández",
      "Sergio Ramos",
      "Gianluigi Buffon",
      "Iker Casillas",
      "Manuel Neuer",
      "Paolo Maldini",
      "Roberto Carlos",
      "David Beckham",
      "Thierry Henry",
      "Karim Benzema",
      "Luis Suárez",
      "Gareth Bale",
      "Antoine Griezmann",
      "Toni Kroos",
      "Virgil van Dijk",
      "Romário",
      "Kaká",
      "Cafú",
      "Carles Puyol",
      "Andrea Pirlo",
      "Francesco Totti",
      "Alessandro Del Piero",
      "Gabriel Batistuta",
      "Didier Drogba",
      "Samuel Eto'o",
      "Wayne Rooney",
      "Steven Gerrard",
      "Frank Lampard",
      "Sergio Agüero",
      "Ángel Di María",
      "Thibaut Courtois",
      "Ter Stegen",
      "Sadio Mané",
      "Son Heung-min",
      "Rodri",
      "Lamine Yamal",
      "Lev Yashin (Araña Negra)",
      "George Best",
      "Michel Platini",
      "Marco van Basten",
      "Ruud Gullit",
      "Lothar Matthäus",
      "Oliver Kahn",
      "Dani Alves",
      "Marcelo",
      "Rivaldo",
      "Figo",
      "Raúl González",
      "Fernando Torres",
      "Eden Hazard",
      "Paul Pogba",
      "N'Golo Kanté",
      "Mesut Özil",
      "Thomas Müller",
      "Bastian Schweinsteiger",
      "Arjen Robben",
      "Robin van Persie",
      "Cesc Fàbregas",
      "Gerard Piqué",
      "Sergio Busquets",
      "David Silva",
      "Radamel Falcao",
      "James Rodríguez",
      "Keylor Navas",
      "Chicharito Hernández",
      "Julián Álvarez",
      "Lautaro Martínez",
      "Emiliano \"Dibu\" Martínez",
      "Alisson Becker",
      "Ederson",
      "Casemiro",
      "Federico Valverde",
      "Pedri",
      "Gavi",
      "Phil Foden",
      "Bukayo Saka",
      "Bernardo Silva",
      "Bruno Fernandes",
      "Pepe",
    ],
    wordHints: {
      "Lionel Messi": "Argentina",
      "Cristiano Ronaldo": "Portugal",
      Pelé: "Brasil",
      "Diego Maradona": "Argentina",
      "Kylian Mbappé": "Francia",
      "Erling Haaland": "Noruega",
      "Neymar Jr.": "Brasil",
      "Zinedine Zidane": "Francia",
      "Ronaldo Nazário (El Fenómeno)": "Brasil",
      Ronaldinho: "Brasil",
      "Johan Cruyff": "Holanda",
      "Franz Beckenbauer": "Alemania",
      "Zlatan Ibrahimović": "Suecia",
      "Robert Lewandowski": "Polonia",
      "Kevin De Bruyne": "Bélgica",
      "Luka Modrić": "Croacia",
      "Vinícius Jr.": "Brasil",
      "Jude Bellingham": "Inglaterra",
      "Harry Kane": "Inglaterra",
      "Mohamed Salah": "Egipto",
      "Andrés Iniesta": "España",
      "Xavi Hernández": "España",
      "Sergio Ramos": "España",
      "Gianluigi Buffon": "Italia",
      "Iker Casillas": "España",
      "Manuel Neuer": "Alemania",
      "Paolo Maldini": "Italia",
      "Roberto Carlos": "Brasil",
      "David Beckham": "Inglaterra",
      "Thierry Henry": "Francia",
      "Karim Benzema": "Francia",
      "Luis Suárez": "Uruguay",
      "Gareth Bale": "Gales",
      "Antoine Griezmann": "Francia",
      "Toni Kroos": "Alemania",
      "Virgil van Dijk": "Holanda",
      Romário: "Brasil",
      Kaká: "Brasil",
      Cafú: "Brasil",
      "Carles Puyol": "España",
      "Andrea Pirlo": "Italia",
      "Francesco Totti": "Italia",
      "Alessandro Del Piero": "Italia",
      "Gabriel Batistuta": "Argentina",
      "Didier Drogba": "Costa de Marfil",
      "Samuel Eto'o": "Camerún",
      "Wayne Rooney": "Inglaterra",
      "Steven Gerrard": "Inglaterra",
      "Frank Lampard": "Inglaterra",
      "Sergio Agüero": "Argentina",
      "Ángel Di María": "Argentina",
      "Thibaut Courtois": "Bélgica",
      "Ter Stegen": "Alemania",
      "Sadio Mané": "Senegal",
      "Son Heung-min": "Corea",
      Rodri: "España",
      "Lamine Yamal": "España",
      "Lev Yashin (Araña Negra)": "Rusia",
      "George Best": "Irlanda",
      "Michel Platini": "Francia",
      "Marco van Basten": "Holanda",
      "Ruud Gullit": "Holanda",
      "Lothar Matthäus": "Alemania",
      "Oliver Kahn": "Alemania",
      "Dani Alves": "Brasil",
      Marcelo: "Brasil",
      Rivaldo: "Brasil",
      Figo: "Portugal",
      "Raúl González": "España",
      "Fernando Torres": "España",
      "Eden Hazard": "Bélgica",
      "Paul Pogba": "Francia",
      "N'Golo Kanté": "Francia",
      "Mesut Özil": "Alemania",
      "Thomas Müller": "Alemania",
      "Bastian Schweinsteiger": "Alemania",
      "Arjen Robben": "Holanda",
      "Robin van Persie": "Holanda",
      "Cesc Fàbregas": "España",
      "Gerard Piqué": "España",
      "Sergio Busquets": "España",
      "David Silva": "España",
      "Radamel Falcao": "Colombia",
      "James Rodríguez": "Colombia",
      "Keylor Navas": "Costa Rica",
      "Chicharito Hernández": "México",
      "Julián Álvarez": "Argentina",
      "Lautaro Martínez": "Argentina",
      "Emiliano \"Dibu\" Martínez": "Argentina",
      "Alisson Becker": "Brasil",
      Ederson: "Brasil",
      Casemiro: "Brasil",
      "Federico Valverde": "Uruguay",
      Pedri: "España",
      Gavi: "España",
      "Phil Foden": "Inglaterra",
      "Bukayo Saka": "Inglaterra",
      "Bernardo Silva": "Portugal",
      "Bruno Fernandes": "Portugal",
      Pepe: "Portugal",
    },
  },
  {
    id: "futbolistas-chilenos",
    name: "Futbolistas Chilenos",
    icon: "🇨🇱",
    iconImage: "/logo-chile.png",
    words: [
      "Alexis Sánchez",
      "Arturo Vidal",
      "Claudio Bravo",
      "Gary Medel",
      "Iván Zamorano",
      "Marcelo Salas",
      "Elías Figueroa",
      "Charles Aránguiz",
      "Mauricio Isla",
      "Jorge Valdivia (El Mago)",
      "Matías Fernández",
      "Eduardo Vargas",
      "Jean Beausejour",
      "Humberto \"Chupete\" Suazo",
      "Carlos Caszely",
      "Leonel Sánchez",
      "Sergio Livingstone (Sapito)",
      "Marcelo Díaz",
      "Gonzalo Jara",
      "Johnny Herrera",
      "Esteban Paredes",
      "Ben Brereton",
      "Erick Pulgar",
      "Gabriel Suazo",
      "Guillermo Maripán",
      "Paulo Díaz",
      "Fabián Orellana",
      "Mark González",
      "David Pizarro",
      "José Luis Sierra (Coto)",
      "Patricio Yáñez",
      "Roberto Rojas (Cóndor)",
      "Lizardo Garrido",
      "Jaime Pizarro",
      "Rubén Martínez",
      "Jorge Aravena (Mortero)",
      "Ivo Basay",
      "Pedro Pablo Hernández",
      "Felipe Gutiérrez",
      "Francisco Silva (Gato)",
      "Edson Puch",
      "José Pedro Fuenzalida (Chapa)",
      "Nicolás Castillo",
      "Carlos Palacios",
      "Darío Osorio",
      "Marcelino Núñez",
      "Brayan Cortés",
      "Gabriel Arias",
      "Claudio Maldonado",
      "Cristián Alvarez (Huaso)",
      "Milovan Mirosevic",
      "Rodrigo Meléndez (Kalule)",
      "Miguel Pinto",
      "Manuel Pellegrini",
      "Lucas Barrios",
      "Sergio Vargas (Superman)",
      "Coca Mendoza",
      "Barticciotto",
      "Waldo Ponce",
      "Pablo Contreras",
      "Rodrigo Tello",
      "Reinaldo Navia",
      "Héctor Tapia",
      "Manuel Neira",
      "Sebastián Rozental",
      "Juvenal Olmos",
      "Luis Jiménez (Mago)",
      "Carlos Villanueva (Piña)",
      "Cristopher Toselli",
      "Ángelo Henríquez",
      "Junior Fernandes",
      "Tomás Alarcón",
      "Vicente Pizarro",
      "Alexander Aravena",
      "Clemente Montes",
      "Lucas Assadi",
      "Damián Pizarro",
      "Luciano Cabral",
      "Eduardo Bonvallet (como figura de fútbol)",
    ],
  },
  {
    id: "champions-league",
    name: "Champions League",
    icon: "🏆",
    iconImage: "/logo-champions.png",
    words: [
      "Real Madrid",
      "FC Barcelona",
      "Manchester City",
      "Liverpool",
      "Bayern Múnich",
      "Paris Saint-Germain (PSG)",
      "Juventus",
      "AC Milan",
      "Inter de Milán",
      "Chelsea",
      "Manchester United",
      "Arsenal",
      "Atlético de Madrid",
      "Borussia Dortmund",
      "Ajax",
      "Benfica",
      "Porto",
      "Napoli",
      "La Orejona (Copa)",
      "Himno de la Champions",
      "Final",
      "Semifinal",
      "Cuartos de Final",
      "Octavos de Final",
      "Fase de Grupos",
      "Sorteo",
      "Bombo 1",
      "Wembley (Estadio icónico)",
      "Santiago Bernabéu",
      "Camp Nou",
      "Allianz Arena",
      "San Siro / Giuseppe Meazza",
      "Anfield",
      "Old Trafford",
      "Parque de los Príncipes",
      "Estambul (Sede histórica)",
      "Balón con estrellas",
      "UEFA",
      "Gol de visitante (histórico)",
      "Tanda de penales",
      "Prórroga / Alargue",
      "Campeón",
      "Subcampeón",
      "Hat-trick",
      "Remontada",
      "El Milagro de Estambul",
      "La Decimocuarta (o la 15)",
      "Pep Guardiola",
      "Carlo Ancelotti",
      "Zinedine Zidane (como DT)",
      "Jürgen Klopp",
      "José Mourinho",
      "Sir Alex Ferguson",
      "Cholo Simeone",
      "Florentino Pérez",
      "Nasser Al-Khelaifi",
      "Jeque (dueños del City/PSG)",
      "Fair Play Financiero",
      "VAR",
      "Árbitro",
      "Tarjeta Roja",
      "Capitán",
      "Hooligans / Ultras",
      "Mosaico (en las gradas)",
      "Martes de Champions",
      "Miércoles de Champions",
      "3:00 PM / 4:00 PM (Hora del partido en Chile)",
      "ESPN / Fox Sports (donde se ve)",
      "Mariano Closs (¡Gol!)",
      "Fernando Niembro (comentarista clásico)",
      "Luis Omar Tapia (Noches Mágicas)",
      "Bayer Leverkusen",
      "RB Leipzig",
      "Sevilla",
      "Tottenham",
      "Roma",
      "Lazio",
      "Olympique de Marsella",
      "Lyon",
      "Mónaco",
      "PSV",
      "Feyenoord",
      "Celtic",
      "Rangers",
      "Galatasaray",
      "Shakhtar Donetsk",
      "Estrella Roja",
      "Dinamo Zagreb",
      "Club Brujas",
      "Salzburgo",
      "Young Boys",
      "Sheriff (la sorpresa)",
      "Newcastle",
      "Aston Villa",
      "Girona (sorpresa reciente)",
      "Título",
      "Medalla",
      "Confeti",
      "Pitazo Final",
      "Gloria Eterna",
    ],
  },
  {
    id: "reggaeton",
    name: "Cantantes de Reggaeton (y Urbano Latino)",
    icon: "🎤",
    words: [
      "Daddy Yankee",
      "Bad Bunny",
      "Karol G",
      "J Balvin",
      "Maluma",
      "Don Omar",
      "Wisin",
      "Yandel",
      "Nicky Jam",
      "Ozuna",
      "Anuel AA",
      "Rauw Alejandro",
      "Feid (Ferxxo)",
      "Arcángel",
      "Tego Calderón",
      "Ivy Queen",
      "Becky G",
      "Natti Natasha",
      "Rosalia",
      "Shakira (sus hits urbanos)",
      "Myke Towers",
      "Sech",
      "Jhayco (Jhay Cortez)",
      "Farruko",
      "Zion & Lennox",
      "Plan B",
      "Chencho Corleone",
      "Maldy",
      "Tito El Bambino",
      "Héctor El Father",
      "Residente / Calle 13",
      "Vico C",
      "El General (Prehistoria)",
      "Young Miko",
      "Manuel Turizo",
      "Camilo",
      "Sebastián Yatra",
      "Piso 21",
      "CNCO",
      "Reik (feat urbanos)",
      "Luis Fonsi (Despacito)",
      "Gente de Zona",
      "Pitbull",
      "De La Ghetto",
      "Ñengo Flow",
      "Cosculluela",
      "Tempo",
      "Jowell & Randy",
      "Alexis & Fido",
      "R.K.M & Ken-Y",
      "Baby Rasta & Gringo",
      "Lunay",
      "Mora",
      "Eladio Carrión",
      "Quevedo",
      "Bizarrap",
      "Duki",
      "Nicki Nicole",
      "Emilia",
      "Tini",
      "Tiago PZK",
      "Trueno",
      "Cris MJ",
      "Pailita",
      "Polimá Westcoast",
      "Pablo Chill-E",
      "Marcianeke",
      "Young Cister",
      "Standly",
      "El Jordan 23",
      "Jere Klein",
      "FloyyMenor",
      "Ak4:20",
      "Julianno Sosa",
      "King Savagge",
      "Paloma Mami",
      "Princesa Alba",
      "Denise Rosenthal",
      "DrefQuila",
      "Harry Nach",
      "Kidd Voodoo",
      "Easykid",
      "Gino Mella",
      "Nio Garcia",
      "Casper Mágico",
      "Darell",
      "Brytiago",
      "Noriel",
      "Bryant Myers",
      "Dalex",
      "Justin Quiles",
      "Lenny Tavárez",
      "Jay Wheeler",
      "Ryan Castro",
      "Blessd",
      "Yovngchimi",
      "Omar Courtz",
      "Saiko",
      "Gonzy",
      "Xavi (Corridos tumbados)",
    ],
  },
  {
    id: "lugares",
    name: "Lugares",
    icon: "📍",
    words: [
      "Restaurante",
      "Hospital",
      "Aeropuerto",
      "Playa",
      "Museo",
      "Escuela",
      "Supermercado",
      "Gimnasio",
      "Biblioteca",
      "Cine",
    ],
  },
  {
    id: "comidas",
    name: "Comidas",
    icon: "🍕",
    words: [
      "Pizza",
      "Hamburguesa",
      "Sushi",
      "Tacos",
      "Ensalada",
      "Pasta",
      "Paella",
      "Ceviche",
      "Croissant",
      "Helado",
    ],
  },
  {
    id: "objetos",
    name: "Objetos",
    icon: "🔧",
    words: [
      "Reloj",
      "Paraguas",
      "Cámara",
      "Libro",
      "Teléfono",
      "Llaves",
      "Gafas",
      "Bolígrafo",
      "Mochila",
      "Lámpara",
    ],
  },
  {
    id: "autos",
    name: "Autos",
    icon: "🚗",
    words: [
      "Toyota Yaris",
      "Nissan V16",
      "Chevrolet Corsa",
      "Suzuki Swift",
      "Kia Morning",
      "Hyundai Accent",
      "Toyota Hilux",
      "Mitsubishi L200",
      "Chevrolet Sail",
      "Suzuki Baleno",
      "Hyundai Tucson",
      "Kia Sportage",
      "Ford F-150",
      "Mazda 3",
      "Volkswagen Gol",
      "Peugeot 206",
      "Peugeot 208",
      "Subaru Impreza",
      "Honda Civic",
      "BMW",
      "Mercedes-Benz",
      "Audi",
      "Ferrari",
      "Lamborghini",
      "Porsche",
      "Jeep Wrangler",
      "Chery Tiggo",
      "MG",
      "JAC",
      "Changan",
      "Great Wall",
      "Volvo",
      "Land Rover",
      "Mini Cooper",
      "Fiat 600",
      "Escarabajo",
      "Colectivo",
      "Micro",
      "Uber",
      "Taxi",
      "Furgón escolar",
      "Camioneta",
      "SUV",
      "Sedan",
      "Hatchback",
      "Descapotable",
      "4x4",
      "Mecánico",
      "Automático",
      "Bencina",
      "Diesel",
      "Auto Eléctrico",
      "Híbrido",
      "Revisión Técnica",
      "Permiso de Circulación",
      "Seguro Obligatorio",
      "Tag",
      "Costanera Norte",
      "Peaje",
      "Taco",
      "Bocina",
      "Neumático",
      "Rueda de repuesto",
      "Gata",
      "Triángulo de seguridad",
      "Chaleco reflectante",
      "Extintor",
      "Volante",
      "Palanca de cambios",
      "Freno de mano",
      "Embrague",
      "Acelerador",
      "Espejo retrovisor",
      "Parabrisas",
      "Limpiaparabrisas",
      "Luces altas",
      "Intermitente",
      "Patente",
      "Llantas",
      "Alerón",
      "Tubo de escape",
      "Batería",
      "Maleta",
      "Radiador",
      "Aire Acondicionado",
      "Apple CarPlay",
      "Waze",
      "Parte",
      "Grúa",
      "Taller",
      "Vulcanización",
      "Estacionamiento",
      "Copec",
      "Automotora",
    ],
  },
  {
    id: "animales",
    name: "Animales",
    icon: "🐾",
    words: [
      "Perro",
      "Gato",
      "Cóndor",
      "Huemul",
      "Puma",
      "Zorro",
      "Llama",
      "Guanaco",
      "Pingüino",
      "Lobo Marino",
      "Ballena",
      "Delfín",
      "Tiburón",
      "León",
      "Tigre",
      "Elefante",
      "Jirafa",
      "Gorila",
      "Mono",
      "Oso",
      "Canguro",
      "Koala",
      "Caballo",
      "Vaca",
      "Toro",
      "Cerdo",
      "Oveja",
      "Cabra",
      "Gallina",
      "Gallo",
      "Pollo",
      "Pato",
      "Pavo",
      "Conejo",
      "Hámster",
      "Ratón",
      "Rata",
      "Murciélago",
      "Águila",
      "Búho",
      "Lechuza",
      "Loro",
      "Canario",
      "Paloma",
      "Gaviota",
      "Pelícano",
      "Flamenco",
      "Cisne",
      "Pájaro Carpintero",
      "Serpiente",
      "Tortuga",
      "Cocodrilo",
      "Lagartija",
      "Iguana",
      "Rana",
      "Pez",
      "Salmón",
      "Reineta",
      "Congrio",
      "Pulpo",
      "Calamar",
      "Medusa",
      "Jaiba",
      "Camarón",
      "Araña",
      "Escorpión",
      "Abeja",
      "Hormiga",
      "Mosca",
      "Zancudo",
      "Mariposa",
      "Polilla",
      "Escarabajo",
      "Cucaracha",
      "Grillo",
      "Saltamontes",
      "Lombriz",
      "Caracol",
      "Babosa",
      "Erizo",
      "Castor",
      "Ornitorrinco",
      "Hipopótamo",
      "Rinoceronte",
      "Cebra",
      "Hiena",
      "Avestruz",
      "T-Rex",
    ],
  },
  {
    id: "tenis",
    name: "Tenis",
    icon: "🎾",
    words: [
      "Marcelo Ríos",
      "Mano de piedra",
      "Fernando González",
      "Nicolás Massú",
      "Nicolás Jarry",
      "Alejandro Tabilo",
      "Cristian Garin",
      "Hans Gildemeister",
      "Patricio Cornejo",
      "Roger Federer",
      "Rafael Nadal",
      "Novak Djokovic",
      "Carlos Alcaraz",
      "Jannik Sinner",
      "Andy Murray",
      "Juan Martín del Potro",
      "Dominic Thiem",
      "Alexander Zverev",
      "Stefanos Tsitsipas",
      "Andre Agassi",
      "Pete Sampras",
      "Gustavo Kuerten",
      "Serena Williams",
      "Venus Williams",
      "Maria Sharapova",
      "Grand Slam",
      "Wimbledon",
      "Roland Garros",
      "US Open",
      "Australian Open",
      "Copa Davis",
      "Ranking ATP",
      "Número 1 del mundo",
      "Top 10",
      "Court Central",
      "Cancha de Arcilla",
      "Cancha Dura",
      "Raqueta",
      "Cuerdas",
      "Red",
      "Juez de Silla",
      "Pasapelotas",
      "Ojo de Halcón",
      "Saque",
      "Ace",
      "Doble Falta",
      "Let",
      "Derecha",
      "Revés",
      "Volea",
      "Smash",
      "Drop Shot",
      "Globo",
      "Slice",
      "Topspin",
      "Winner",
      "Error no forzado",
      "Game",
      "Set",
      "Match Point",
      "Set Point",
      "Break Point",
      "Break",
      "Tie Break",
      "Super Tie Break",
      "Deuce",
      "Ventaja",
      "15, 30, 40",
      "Singles",
      "Dobles",
      "Dobles Mixto",
      "Medalla de Oro",
      "Atenas 2004",
      "Entrenador",
      "Kinesiólogo",
      "Toalla",
      "Muñequera",
    ],
  },
  {
    id: "real-madrid",
    name: "Real Madrid",
    icon: "⚽",
    iconImage: "/logo-real-madrid.png",
    words: [
      "Emilio Butragueño",
      "Hugo Sánchez",
      "Michel",
      "Fernando Hierro",
      "Raúl González",
      "Roberto Carlos",
      "Iker Casillas",
      "Zinedine Zidane",
      "Ronaldo Nazário",
      "David Beckham",
      "Sergio Ramos",
      "Cristiano Ronaldo",
      "Karim Benzema",
      "Luka Modrić",
      "Toni Kroos",
      "Gareth Bale",
      "Marcelo",
      "Thibaut Courtois",
      "Vinicius Jr",
      "Jude Bellingham",
      "Iván Zamorano",
      "Claude Makélélé",
      "Ruud van Nistelrooy",
      "Kaká",
      "Xabi Alonso",
      "Pepe",
      "Casemiro",
      "Federico Valverde",
    ],
  },
  {
    id: "barcelona",
    name: "Barcelona",
    icon: "🔵",
    iconImage: "/logo-barcelona.png",
    words: [
      "Diego Maradona",
      "Ronald Koeman",
      "Hristo Stoichkov",
      "Michael Laudrup",
      "Ronaldo Nazário",
      "Rivaldo",
      "Xavi Hernández",
      "Andrés Iniesta",
      "Carles Puyol",
      "Lionel Messi",
      "Neymar Jr",
      "Luis Suárez",
      "Gerard Piqué",
      "Sergio Busquets",
      "Pedri",
      "Gavi",
      "Johan Cruyff",
      "Samuel Eto'o",
      "Thierry Henry",
      "David Villa",
      "Victor Valdés",
      "Dani Alves",
      "Jordi Alba",
      "Marc-André ter Stegen",
      "Frenkie de Jong",
      "Robert Lewandowski",
    ],
  },
  {
    id: "manchester-united",
    name: "Manchester United",
    icon: "🔴",
    iconImage: "/logo-manchester-united.png",
    words: [
      "Bryan Robson",
      "Eric Cantona",
      "Roy Keane",
      "David Beckham",
      "Paul Scholes",
      "Ryan Giggs",
      "Wayne Rooney",
      "Cristiano Ronaldo",
      "Ruud van Nistelrooy",
      "Rio Ferdinand",
      "Nemanja Vidić",
      "Edwin van der Sar",
      "Peter Schmeichel",
      "Ole Gunnar Solskjær",
      "Andy Cole",
      "Dwight Yorke",
      "Gary Neville",
      "Patrice Evra",
      "Michael Carrick",
      "Robin van Persie",
      "Zlatan Ibrahimović",
      "Bruno Fernandes",
      "Marcus Rashford",
      "Sir Alex Ferguson",
    ],
  },
  {
    id: "river-plate",
    name: "River Plate",
    icon: "🔴",
    iconImage: "/logo-river.png",
    words: [
      "Enzo Francescoli",
      "Ariel Ortega",
      "Marcelo Gallardo",
      "Pablo Aimar",
      "Andrés D'Alessandro",
      "Radamel Falcao",
      "Gonzalo Higuaín",
      "Javier Saviola",
      "Fernando Cavenaghi",
      "Juan Pablo Sorín",
      "Roberto Ayala",
      "Marcelo Salas",
      "Claudio Caniggia",
      "Ramón Díaz",
      "Gabriel Batistuta",
      "Franco Armani",
      "Enzo Fernández",
      "Julian Álvarez",
    ],
  },
  {
    id: "boca-juniors",
    name: "Boca Juniors",
    icon: "💙",
    iconImage: "/logo-boca.png",
    words: [
      "Diego Maradona",
      "Gabriel Batistuta",
      "Juan Román Riquelme",
      "Martín Palermo",
      "Carlos Tevez",
      "Fernando Gago",
      "Claudio Caniggia",
      "Oscar Córdoba",
      "Walter Samuel",
      "Roberto Abbondanzieri",
      "Rodrigo Palacio",
      "Carlos Bianchi",
      "Guillermo Barros Schelotto",
      "Sebastián Battaglia",
      "Nicolás Burdisso",
      "Éver Banega",
      "Darío Benedetto",
      "Frank Fabra",
    ],
  },
  {
    id: "universidad-catolica",
    name: "Jugadores de Universidad Católica (De siempre)",
    icon: "🛡️",
    iconImage: "/logo-uc.png",
    words: [
      "Mario Lepe",
      "Alberto Acosta (Beto)",
      "Néstor Gorosito (Pipo)",
      "Milovan Mirosevic",
      "Gary Medel",
      "Fernando Zampedri",
      "José María Buljubasich (Tati)",
      "Raimundo Tupper (Mumo)",
      "Ignacio Prieto",
      "Alberto Foullioux (Tito)",
      "Sergio Livingstone (Sapito)",
      "Jorge Aravena (Mortero)",
      "Sebastián Rozental",
      "Juvenal Olmos",
      "Cristián Álvarez (Huaso)",
      "Nicolás Castillo",
      "Diego Buonanotte",
      "Matías Dituro",
      "Darío Botinelli",
      "Darío Conca",
      "Damián Diaz",
      "José Pedro Fuenzalida (Chapa)",
      "Lucas Pratto",
      "Jorge Ormeño",
      "Facundo Imboden",
      "Ricardo Lunari",
      "Gerardo Reinoso (La Vieja)",
      "Marco Antonio Figueroa (Fantasma)",
      "Patricio Toledo",
      "Andrés Romero (Moto)",
      "Dante Poli",
      "Mark González",
      "Felipe Gutiérrez",
      "Francisco Silva (Gato)",
      "Enzo Roco",
      "Alfonso Parot",
      "Germán Lanaro",
      "Luciano Aued",
      "César Pinares",
      "Matias Palaveccino",
      "Edson Puch",
      "Alexander Aravena",
      "Marcelino Núñez",
      "Gonzalo Tapia",
      "Clemente Montes",
      "Ignacio Saavedra",
      "Benjamín Kuscevic",
      "Guillermo Maripán",
      "Erick Pulgar",
      "Jeisson Vargas",
      "Roberto Gutiérrez (Pájaro)",
      "Michael Ríos",
      "Fernando Meneses",
      "Rodrigo Barrera",
      "Eduardo Bonvallet",
      "Iván Vásquez",
      "Hans Martínez",
      "Cristopher Toselli",
      "Paulo Garcés",
      "Ismael Sosa",
      "Tomás Costa",
      "Fernando Cordero (Chiki)",
      "Duvier Riascos",
      "Santiago Silva (Tanque)",
      "Francisco Arrué",
      "José Luis Villanueva",
      "Hugo Droguett",
      "Eros Pérez",
      "Valber Huerta",
      "Branco Ampuero",
      "Eugenio Mena",
      "Mauricio Isla",
      "Cuevas (Cimbi)",
      "Francisco Rossel",
      "Tiago Nunes",
      "Daniel Garnero",
    ],
  },
  {
    id: "alamicos-rfc",
    name: "Alamicos R.F.C",
    icon: "⚽",
    iconImage: "/logo-alamicos.png",
    words: [
      "La alamo",
      "Maxi Gomez",
      "Jose Saenz",
      "Bisku",
      "Gianca",
      "RuCio",
      "Nachito",
      "Fiera",
      "Pipe",
      "Chago",
      "Tata",
      "Memo",
      "Goldelmemo",
      "Dani",
      "Tricampeones",
      "Liga 7",
      "Palestino",
      "Country Club",
      "Juampi",
      "JJ Aguero",
      "Feño",
      "Araya",
      "Toti",
      "Marito",
      "Nacho Hasbun",
      "Jordan",
      "Nico Ruiseñor",
      "Nacho Elguera",
      "Rorro",
      "Casitas Cordero",
      "Guiller Pizarro",
      "Gonza Aliaga",
      "Josemi",
      "Rodri",
      "Patricia",
      "Sofia",
      "Sola (remix)",
      "La Player (Zion y lennox)",
      "Ester Exposito",
      "Magui Corceiro",
      "Correa",
      "Colegio Alcazar",
    ],
  },
  {
    id: "famosos-chilenos",
    name: "Famosos Chilenos (Personajes de TV, Historia y Farándula)",
    icon: "🇨🇱",
    words: [
      "Don Francisco (Mario Kreutzberger)",
      "Pedro Pascal",
      "Felipe Camiroaga",
      "Cecilia Bolocco",
      "Stefan Kramer",
      "Bombo Fica",
      "Mon Laferte",
      "Violeta Parra",
      "Pablo Neruda",
      "Gabriela Mistral",
      "Leonardo Farkas",
      "Pamela Díaz (La Fiera)",
      "Raquel Argandoña",
      "Pancho Saavedra",
      "Julio César Rodríguez",
      "Martín Cárcamo",
      "Diana Bolocco",
      "José Antonio Neme",
      "Tonka Tomicic",
      "Lucho Jara",
      "Myriam Hernández",
      "Coco Legrand",
      "Naya Fácil",
      "Jorge González (Los Prisioneros)",
      "Beto Cuevas (La Ley)",
      "Benjamín Vicuña",
      "Jorge Zabaleta",
      "Pedro Engel",
      "Doctor Simi",
      "Guru Guru",
      "Tulio Triviño",
      "Juan Carlos Bodoque",
      "Condorito",
      "Arturo Prat",
      "Bernardo O'Higgins",
      "El merluso",
      "Sebastián Piñera",
      "Michelle Bachelet",
      "Augusto Pinochet",
      "Salvador Allende",
      "Evelyn Matthei",
      "Camila Vallejo",
      "José Antonio Kast",
      "Antonio Vodanovic",
      "Kike Morandé",
      "Marlen Olivari",
      "Willy Sabor",
      "Paty Maldonado",
      "Jordi Castell",
      "Cristián Sánchez",
      "Soledad Onetto",
      "Mónica Rincón",
      "Daniel Matamala",
      "Fabrizio Copano",
      "Sergio Freire",
      "Pedro Ruminot",
      "Paul Vásquez (El Flaco)",
      "Mauricio Medina (El Indio)",
      "Dino Gordillo",
      "Álvaro Salas",
      "Iván Arenas (Profesor Rossa)",
      "Carolina Arregui",
      "Paz Bascuñán (Soltera otra vez)",
      "Fernando Godoy (Nachito)",
      "Pollo Castillo",
      "Zafrada",
      "El Tarro",
      "Chimuelo",
      "Negro Piñera",
      "Edmundo Varas",
      "Junior Playboy",
      "Kenita Larraín",
      "Coté López",
      "Mago Valdivia",
      "Sammy Reyes",
      "Emilia Dides",
    ],
  },
  {
    id: "peliculas",
    name: "Películas Famosas (Mundiales y Chilenas)",
    icon: "🎬",
    words: [
      "Titanic",
      "Avatar",
      "El Rey León",
      "Shrek",
      "Toy Story",
      "Harry Potter",
      "El Señor de los Anillos",
      "Star Wars (La Guerra de las Galaxias)",
      "Avengers (Los Vengadores)",
      "Spiderman (El Hombre Araña)",
      "Batman (El Caballero de la Noche)",
      "Superman",
      "Joker (El Guasón)",
      "Barbie",
      "Oppenheimer",
      "Rápido y Furioso",
      "Matrix",
      "Volver al Futuro",
      "Jurassic Park",
      "E.T. El Extraterrestre",
      "Mi Pobre Angelito",
      "El Padrino",
      "Rocky",
      "Terminator",
      "Forrest Gump",
      "Gladiador",
      "Piratas del Caribe",
      "Misión Imposible",
      "Top Gun",
      "Indiana Jones",
      "James Bond (007)",
      "Los Juegos del Hambre",
      "Crepúsculo",
      "50 Sombras de Grey",
      "El Diario de una Princesa",
      "El Diablo Viste a la Moda",
      "Legalmente Rubia",
      "La La Land",
      "Bohemia Rhapsody",
      "Intensamente (Inside Out)",
      "Frozen",
      "Buscando a Nemo",
      "Los Increíbles",
      "Monsters Inc.",
      "Cars",
      "Ratatouille",
      "Up",
      "Wall-E",
      "Minions / Mi Villano Favorito",
      "Kung Fu Panda",
      "Era del Hielo",
      "Madagascar",
      "El Grinch",
      "Charlie y la Fábrica de Chocolate",
      "El Exorcista",
      "El Conjuro",
      "Chucky",
      "Freddy Krueger",
      "Inception (El Origen)",
      "Interstellar",
      "El Lobo de Wall Street",
      "Pulp Fiction",
      "Machuca",
      "Stefan v/s Kramer",
      "El Chacotero Sentimental",
      "Papelucho y el Marciano",
    ],
    wordHints: {
      Titanic: "Barco",
      Avatar: "Ciencia ficción",
      "El Rey León": "León",
      Shrek: "Ogro",
      "Toy Story": "Juguetes",
      "Harry Potter": "Magia",
      "El Señor de los Anillos": "Anillo",
      "Star Wars (La Guerra de las Galaxias)": "Espacio",
      "Avengers (Los Vengadores)": "Superhéroes",
      "Spiderman (El Hombre Araña)": "Araña",
      "Batman (El Caballero de la Noche)": "Murciélago",
      Superman: "Capa",
      "Joker (El Guasón)": "Payaso",
      Barbie: "Muñeca",
      Oppenheimer: "Bomba",
      "Rápido y Furioso": "Autos",
      Matrix: "Pastilla",
      "Volver al Futuro": "DeLorean",
      "Jurassic Park": "Dinosaurios",
      "E.T. El Extraterrestre": "Alien",
      "Mi Pobre Angelito": "Ladrón",
      "El Padrino": "Mafia",
      Rocky: "Boxeo",
      Terminator: "Robot",
      "Forrest Gump": "Correr",
      Gladiador: "Roma",
      "Piratas del Caribe": "Pirata",
      "Misión Imposible": "Espía",
      "Top Gun": "Avión",
      "Indiana Jones": "Arqueólogo",
      "James Bond (007)": "Espía",
      "Los Juegos del Hambre": "Distrito",
      Crepúsculo: "Vampiro",
      "50 Sombras de Grey": "Romance",
      "El Diario de una Princesa": "Princesa",
      "El Diablo Viste a la Moda": "Moda",
      "Legalmente Rubia": "Rubia",
      "La La Land": "Jazz",
      "Bohemia Rhapsody": "Queen",
      "Intensamente (Inside Out)": "Emociones",
      Frozen: "Hielo",
      "Buscando a Nemo": "Pez",
      "Los Increíbles": "Superhéroes",
      "Monsters Inc.": "Monstruos",
      Cars: "Autos",
      Ratatouille: "Rata",
      Up: "Globo",
      "Wall-E": "Robot",
      "Minions / Mi Villano Favorito": "Amarillo",
      "Kung Fu Panda": "Panda",
      "Era del Hielo": "Mamut",
      Madagascar: "León",
      "El Grinch": "Navidad",
      "Charlie y la Fábrica de Chocolate": "Chocolate",
      "El Exorcista": "Demonio",
      "El Conjuro": "Fantasma",
      Chucky: "Muñeco",
      "Freddy Krueger": "Pesadilla",
      "Inception (El Origen)": "Sueño",
      Interstellar: "Espacio",
      "El Lobo de Wall Street": "Dinero",
      "Pulp Fiction": "Gángsters",
      Machuca: "Colegio",
      "Stefan v/s Kramer": "Comedia",
      "El Chacotero Sentimental": "Radio",
      "Papelucho y el Marciano": "Marciano",
    },
  },
];
